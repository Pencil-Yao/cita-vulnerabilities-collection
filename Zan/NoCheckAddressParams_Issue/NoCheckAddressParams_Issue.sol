...
contract Test {
    ...
    function bad1(address addr1, address addr2) external {
        //leak
        addr1.call{ value: 1 }("");
        addr2.call{ value: 1 }("");
    }

    function bad2(address addr1, address addr2) external {
        //leak
        if (addr1 != address(0x0)) {
            revert("addr1 should not be zero address");
        }
    ...
    function bad3(address addr1, address addr2) external {
        //leak
        if (addr2 != 0x0000000000000000000000000000000000000000) {
            revert("addr2 should not be zero address");
        }
    ...
    function good4(address to, address from, uint256 value) external returns (bool) {
        bool isSuccess = interCalled1(to, from, value);
        return isSuccess;
    }

    function interCalled1(address to1, address from1, uint256 value) internal returns (bool) {
        require(to1 != address(0) || from1 != address(0) || to1 != address(this));
