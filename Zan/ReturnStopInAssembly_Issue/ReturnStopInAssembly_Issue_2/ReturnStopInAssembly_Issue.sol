...
contract Test is ReentrancyGuard {
    ...
    function bad1(bool success, bytes calldata resultData) external nonReentrant {
        //leak
        if (!success) {
    ...
    function bad2(bool success, bytes calldata resultData) external nonReentrant {
        //leak
        if (!success) {
            _revertWithData(resultData);
        }
    ...
    function good1(bool success, bytes calldata resultData) external nonReentrant {
        if (!success) {
            _revertWithData(resultData);
        }
    }
    ...
    function good3(bool success, bytes calldata resultData) external nonReentrant {
        if (!success) {
