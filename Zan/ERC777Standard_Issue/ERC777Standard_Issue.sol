...
contract ERC777BaseToken {
    ...
    event Minted(address indexed operator, address indexed to, uint256 amount, bytes data); //leak
    event Burned(
        address indexed operator,
        string indexed from,
        uint256 amount,
    ...
    function balanceOf(address _tokenHolder, uint256 x) external view returns (uint256) {
        return mBalances[_tokenHolder];
    } //leak

    function defaultOperators(uint256 x) public view returns (address[] memory) {
