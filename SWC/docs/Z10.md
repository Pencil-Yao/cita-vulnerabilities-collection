...
根据 EIP-712 (https://github.com/ethereum/EIPs/blob/master/EIPS/eip-712.md)，个人签名应编码为：`encode(b : 𝔹⁸ⁿ) = “\x19Ethereum Signed Message:\n” ‖ len(b) ‖ b`，其中 `len(b)` 是 b 的 ascii-decimal 编码的字节数。请检查 `len(b)` 是否为 b 的长度。
