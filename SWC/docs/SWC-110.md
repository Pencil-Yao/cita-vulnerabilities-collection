...
Solidity 的`assert()`函数用于不变量的断言，正常运行的代码*永远*不会到达失败的assert语句。可达到的断言可能意味着以下两种情况之一：
...
[CWE-670：不正确的控制流程实现](https://cwe.mitre.org/data/definitions/670.html)
...
- [revert/assert/require以及EVM REVERT操作码的使用](https://media.consensys.net/when-to-use-revert-assert-and-require-in-solidity-61fb2c0e5a57)
