...
汇编中的 `return` 或 `stop` 语句会直接终止程序的执行，因此 `modifier` 中占位符 `_` 后面的语句不会被执行，从而导致重入锁死锁等严重问题。

## 修改建议

建议去掉函数的anti-reentrancy modifier，或去掉汇编中的`return`或`stop`语句。
...
ReturnStopInAssembly_Issue_2.sol
### ReentrancyGuard.sol
